...
TCN_IMPLEMENT_CALL(jint, SSLExt, setSNI)(TCN_STDARGS, jlong tcsock, jbyteArray buf, jint len)
{
    ...
    (*e)->GetByteArrayRegion(e, buf, 0, len, (jbyte *) bytes);
        ...
        const unsigned char *in, unsigned int inlen, void *arg)
{
    ...
    if (tcsslctx->npn != 0) {
        int status = SSL_select_next_proto(out, outlen, in, inlen,
                (const unsigned char *) tcsslctx->npn,
                strlen(tcsslctx->npn));
        switch (status) {
        ...
        jbyteArray buf, jint len)
{
    ...
    (*e)->GetByteArrayRegion(e, buf, 0, len, (jbyte *) &sslctx->npn[0]);
...
TCN_IMPLEMENT_CALL(jint, SSLExt, getNPN)(TCN_STDARGS, jlong tcsock, jbyteArray buf)
{
    ...
    (*e)->SetByteArrayRegion(e, buf, 0, npn_len, (jbyte *) npn);
